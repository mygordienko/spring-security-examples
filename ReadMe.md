# Spring Security examples

* [Username and password, default login page](simple-form-login/ReadMe.md)
* [OIDC login, no login page](oidc-simple-client/ReadMe.md)
* [OIDC Auth Server](oidc-auth-server/ReadMe.md)
* [OAuth2 Auth Server, custom login page, client management](oauth2-auth-server/ReadMe.md))
* [LDAP login](ldap-simple-auth/ReadMe.md)
* [OAuth2 login authorization code flow, token relay to resource server](oauth2-client/ReadMe.md))
  * [client](oauth2-client/ReadMe.md)
  * [auth server](oauth2-auth-server/ReadMe.md)
  * [resource server](oauth2-resource-server/ReadMe.md)
* OTT (One-Time Token) login
* Passkey login
* Multi-factor login (password + ott)